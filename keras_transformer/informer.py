...
class ProbAttentionLayer(layers.Layer):
  ...
  def __init__(self, params: dict, **kwargs):
    ...
    self.n_heads = params.get("n_heads", 2)
    ...
    self.seed = ops.random.SeedGenerator()
  ...
  def build(self, input_shape: tuple[tuple[int, ...], ...]):
    ...
    self.query_proj = layers.Dense(self.n_heads * self.d_keys)
    self.key_proj = layers.Dense(self.n_heads * self.d_keys)
    self.value_proj = layers.Dense(self.n_heads * self.d_values)
  ...
  def _prob_qk(self,
               ...
               keys: Tensor,
               sample_k: int,
               n_top: int) -> tuple[Tensor, Tensor]:
    _, _, q_len, _ = queries.shape
    _, _, k_len, _ = keys.shape

    # Calculate sampled Q_k
    k_expand = ops.tile(keys[:, :, None, :, :],
                        (1, 1, q_len, 1, 1))
    index_sample = ops.random.randint((q_len, sample_k),
                                      0, k_len,
                                      seed=self.seed)
    k_sample = k_expand[:, :, ops.arange(q_len)[:, None], index_sample, :]
    q_k_sample = ops.matmul(queries[..., None, :],
                            ops.transpose(k_sample, (0, 1, 2, 4, 3)))[..., 0, :]
  ...
  def _prob_mask(self, length: int, index: Tensor, scores: Tensor):
    mask = ops.triu(
        ops.ones((length, scores.shape[-1]),
                 dtype="bool"), 1)[None, None, ...]
  ...
  def call(self, inputs: tuple[Tensor, Tensor, Tensor]
    ...
    scores_top, index = self._prob_qk(queries=queries,
                                      keys=keys,
                                      sample_k=u_part,
                                      n_top=u)
    ...
    if keras.backend.backend() == "jax":
      context = context.at[
          ops.arange(bs)[:, None, None],
          ops.arange(n_heads)[None, :, None],
          index, :].set(attn_values)
    else:
      context[ops.arange(bs)[:, None, None],
              ops.arange(n_heads)[None, :, None],
              index, :] = attn_values
    ...
    out = self.out_proj(
        ops.reshape(
            context,
            (-1, q_len, n_heads * self.d_values)
        )
    )

    if self.output_attention:
      attn_ext = ops.ones([bs, n_heads, v_len, v_len], dtype=attn.dtype) / v_len

      if keras.backend.backend() == "jax":
        attn_ext = attn_ext.at[
            ops.arange(bs)[:, None, None],
            ops.arange(n_heads)[None, :, None],
            index, :].set(attn)
      else:
        attn_ext[ops.arange(bs)[:, None, None],
                 ops.arange(n_heads)[None, :, None],
                 index, :] = attn
...
class ProbDecoderLayer(layers.Layer):
  ...
  def build(self, input_shape: tuple[tuple[int, ...], ...]):
    ...
    n_heads = self.params.get("n_heads", 2)
    self.cross_attn = layers.MultiHeadAttention(
        num_heads=n_heads,
        key_dim=self.params.get("d_keys",
                                input_shape[0][-1] // n_heads)
    )
