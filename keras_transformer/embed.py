...
class PositionalEmbedding(layers.Layer):
  ...
  def call(self, inputs: Tensor):
    return ops.take_along_axis(self.pos_matrix,
                               ops.clip(
                                   inputs, 0, self.input_dim - 1)[..., None],
                               axis=-2)[..., :self.output_dim]
  ...
  def __init__(self, d_model: PositiveInt,
    ...
    if self.embed_type == "temporal":
      ...
      if self.embed_type == "positional":
        self.time_embed = PositionalEmbedding(10000, d_model)
      elif self.embed_type == "fixed":
        self.time_embed = layers.Embedding(10000, d_model)
      else:
        self.time_embed = None
...
class DataEmbedding(layers.Layer):
  ...
  def __init__(self,
    ...
    # Create the embedding layers
    self.value_embedding = layers.Conv1D(filters=d_model,
                                         kernel_size=3,
                                         padding="same",
    ...
    self.temporal_embedding = \
        TemporalEmbedding(d_model=d_model,
...
def timestamps_to_marks(timestamps: list[str],
